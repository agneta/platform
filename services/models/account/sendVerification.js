  ...
  Model.sendVerification = function(options) {
    ...
    return options.account
      .verify({
        ...
        subject:
          options.subject || app.lng('account.registeredSubject', options.req),
        language: language,
        req: options.req,
    ...
    return Promise.resolve()
      .then(function() {
        assert(
          ...
          'options required when calling user.verify()'
        );
        assert(options.verifyHref, 'You must supply a verification href');
        assert(
          options.type,
          'You must supply a verification type (options.type)'
        );
        assert(options.type === 'email', 'Unsupported verification type');
        assert(
          options.to || this.email,
          'Must include options.to when calling user.verify() or the user must have an email property'
        );

        var tokenGenerator =
          options.generateVerificationToken || Model.generateVerificationToken;
